...
type stubPushbackApplyMgr struct {
	result matches.PushbackPreviewResult
	err    error
}

func (s *stubPushbackApplyMgr) Apply(_ context.Context, _ matches.PushbackPreviewRequest) (matches.PushbackPreviewResult, error) {
	return s.result, s.err
}

func applyDeps(mgr PushbackApplier) Dependencies {
	return Dependencies{
		HandicapSvc:      &noopRecommender{},
		RuleMgr:          &noopRuleManager{},
		LeagueMgr:        &noopLeagueMgr{},
		...
		TeamMgr:          &noopTeamMgr{},
		SeasonMgr:        &noopSeasonMgr{},
		PushbackApplyMgr: mgr,
	}
}

func doPushbackApply(t *testing.T, mgr PushbackApplier, seasonID string, body any) *httptest.ResponseRecorder {
	t.Helper()
	b, _ := json.Marshal(body)
	req := httptest.NewRequest(http.MethodPost,
		"/api/seasons/"+seasonID+"/schedule/pushback-apply",
		bytes.NewReader(b))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()
	mux := http.NewServeMux()
	Register(mux, t.TempDir(), applyDeps(mgr))
	mux.ServeHTTP(w, req)
	return w
}

func TestPushbackApply_200_ReturnsResult(t *testing.T) {
	date := "2026-10-13"
	mgr := &stubPushbackApplyMgr{
		result: matches.PushbackPreviewResult{
			Shifted: []matches.ShiftedMatch{
				{ID: 1, WeekNumber: 5, NewWeekNumber: 6, HomeTeamID: 1, AwayTeamID: 2, NewMatchDate: &date},
			},
	...
	w := doPushbackApply(t, mgr, "1", map[string]int{"cutoff_week": 5, "weeks_to_add": 1})
	if w.Code != http.StatusOK {
		t.Fatalf("want 200, got %d: %s", w.Code, w.Body.String())
	}
	var got matches.PushbackPreviewResult
...
func TestPushbackApply_400_InvalidCutoff(t *testing.T) {
	mgr := &stubPushbackApplyMgr{
		err: domainerr.New("PUSHBACK_INVALID_CUTOFF", domainerr.InvalidInput, "cutoff_week must be at least 1"),
	}
	w := doPushbackApply(t, mgr, "1", map[string]int{"cutoff_week": 0, "weeks_to_add": 1})
	if w.Code != http.StatusBadRequest {
		t.Errorf("want 400, got %d", w.Code)
	}
}

func TestPushbackApply_400_InvalidWeeksToAdd(t *testing.T) {
	mgr := &stubPushbackApplyMgr{
		err: domainerr.New("PUSHBACK_INVALID_WEEKS_TO_ADD", domainerr.InvalidInput, "weeks_to_add must be at least 1"),
	}
	w := doPushbackApply(t, mgr, "1", map[string]int{"cutoff_week": 3, "weeks_to_add": 0})
	if w.Code != http.StatusBadRequest {
		t.Errorf("want 400, got %d", w.Code)
	}
}

func TestPushbackApply_409_ClosedWeeks(t *testing.T) {
	mgr := &stubPushbackApplyMgr{
		err: domainerr.New("PUSHBACK_HAS_CLOSED_WEEKS", domainerr.Conflict, "cannot pushback: closed weeks at or after cutoff"),
	}
	w := doPushbackApply(t, mgr, "1", map[string]int{"cutoff_week": 5, "weeks_to_add": 1})
...
func TestPushbackApply_404_SeasonNotFound(t *testing.T) {
	mgr := &stubPushbackApplyMgr{
		err: domainerr.New("PUSHBACK_SEASON_NOT_FOUND", domainerr.NotFound, "season not found"),
	}
	w := doPushbackApply(t, mgr, "99", map[string]int{"cutoff_week": 1, "weeks_to_add": 1})
	if w.Code != http.StatusNotFound {
		t.Errorf("want 404, got %d", w.Code)
	}
}

func TestPushbackApply_400_InvalidSeasonID(t *testing.T) {
	mgr := &stubPushbackApplyMgr{}
	w := doPushbackApply(t, mgr, "notanid", map[string]int{"cutoff_week": 1, "weeks_to_add": 1})
	if w.Code != http.StatusBadRequest {
		t.Errorf("want 400 for non-numeric season id, got %d", w.Code)
	}
}
